...
class DroneVideoDisplay(QtGui.QMainWindow):
	...
	def __init__(self):
		...
		self.pub_z = rospy.Publisher('zpixel', Int32, queue_size=300)  # publish desired path index of integer type
		self.z_value = Int32() # Container for desired path index of integer type
		self.z_value = 0
	...
	def SaveImage(self,index=0):
		...
		if self.imageLock.acquire():
			try:
				...
				output = cv_image.copy() # copy image
				image_blur = cv2.GaussianBlur(cv_image, (3, 3), 0) # smoothen image using gaussian blur filter
				hsv_image = cv2.cvtColor(image_blur,cv2.COLOR_BGR2HSV) # convert to HSV
				lower_blue = np.array([80,0,5]) # lower value for blue mask
				upper_blue = np.array([110,255,255]) # upper value for blue mask
				mask = cv2.inRange(hsv_image,lower_blue,upper_blue) # mask to filter out blue colour hula hoop
				res = cv2.bitwise_and(cv_image,cv_image,mask=mask) # apply mask
				gray = cv2.cvtColor(res, cv2.COLOR_BGR2GRAY) # convert to grey scale
				circles = cv2.HoughCircles(gray,cv2.cv.CV_HOUGH_GRADIENT,1,640,param1=50,param2=30,minRadius=70,maxRadius=0) # detect circle
				self.z_value = 0 # if no circle found, set height offset to 0
				# if circle found
				if circles is not None: 
					print('Circle detected')
					circles = np.round(circles[0, :]).astype("int") # store center position and radius of circle

					# loop over the (x, y) coordinates and radius of the circles
					for (x, y, r) in circles:
						# draw the circle in the output image, then draw a rectangle
						# corresponding to the center of the circle
						self.z_value = 180 -  y # image plane origin's height brought to middle of image
						cv2.circle(output, (x, y), r, (0, 255, 0), 4) # draw circle detected
						cv2.rectangle(output, (x - 5, y - 5), (x + 5, y + 5), (0, 128, 255), -1) # draw center

				self.pub_z.publish(self.z_value) # publish height offset to correct for

			except CvBridgeError as e:
				print "Image conversion failed: %s" % e
			# TODO use cv2.imwrite function to write images to local disk
			finally:
				cv2.imwrite("/home/yukun/index"+str(index)+".png",output)
